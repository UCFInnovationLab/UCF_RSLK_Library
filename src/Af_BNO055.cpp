 ...
 * @file Af_BNO055.cpp
 *
 *  @mainpage Af BNO055 Orientation Sensor
 *
 *  @section intro_sec Introduction
 *
 *  This is a library for the BNO055 orientation sensor
 *
 *  Designed specifically to work with the Af BNO055 9-DOF Breakout.
 *
 ...
 *
 *  Af invests time and resources providing this open source code,
 *  please support Af andopen-source hardware by purchasing products
 *  from Af!
 *
 *  @section author Author
 *
 *  K.Townsend (Af Industries)
 *
...
/*!
 *  @brief  Instantiates a new Af_BNO055 class
 *  @param  sensorID
 *          sensor ID
 *  @param  address
 *          i2c address
 ...
 *          Wire object
 */
Af_BNO055::Af_BNO055(int32_t sensorID, uint8_t address,
                                 TwoWire *theWire) {
// BNO055 clock stretches for 500us or more!
  ...
  _sensorID = sensorID;
  i2c_dev = new Af_I2CDevice(address, theWire);
}
 ...
 */
bool Af_BNO055::begin(af_bno055_opmode_t mode) {
 ...
 *            OPERATION_MODE_NDOF]
 */
void Af_BNO055::setMode(af_bno055_opmode_t mode) {
  _mode = mode;
 ...
 */
af_bno055_opmode_t Af_BNO055::getMode() {
  return (af_bno055_opmode_t)read8(BNO055_OPR_MODE_ADDR);
}
 ...
 *           REMAP_CONFIG_P7]
 */
void Af_BNO055::setAxisRemap(
    af_bno055_axis_remap_config_t remapcode) {
  af_bno055_opmode_t modeback = _mode;
 ...
 *           REMAP_SIGN_P7]
 */
void Af_BNO055::setAxisSign(af_bno055_axis_remap_sign_t remapsign) {
  af_bno055_opmode_t modeback = _mode;
 ...
 */
void Af_BNO055::setExtCrystalUse(boolean usextal) {
  af_bno055_opmode_t modeback = _mode;
 ...
 *           system error info
 */
void Af_BNO055::getSystemStatus(uint8_t *system_status,
 ...
 *          revision info
 */
void Af_BNO055::getRevInfo(af_bno055_rev_info_t *info) {
  uint8_t a, b;

  memset(info, 0, sizeof(af_bno055_rev_info_t));
 ...
 */
void Af_BNO055::getCalibration(uint8_t *sys, uint8_t *gyro,
 ...
 */
imu::Vector<3> Af_BNO055::getVector(af_vector_type_t vector_type) {
  imu::Vector<3> xyz;
  uint8_t buffer[6];
  memset(buffer, 0, 6);
  ...
  readLen((af_bno055_reg_t)vector_type, buffer, 6);
 ...
 */
imu::Quaternion Af_BNO055::getQuat() {
  uint8_t buffer[8];
  memset(buffer, 0, 8);
 ...
 *          Sensor description
 */
void Af_BNO055::getSensor(sensor_t *sensor) {
 ...
 */
bool Af_BNO055::getEvent(sensors_event_t *event) {
  /* Clear the event */
  ...
  imu::Vector<3> euler = getVector(Af_BNO055::VECTOR_EULER);
...
bool Af_BNO055::getEvent(sensors_event_t *event,
  ...
  imu::Vector<3> vec;
  if (vec_type == Af_BNO055::VECTOR_LINEARACCEL) {
    event->type = SENSOR_TYPE_LINEAR_ACCELERATION;
    vec = getVector(Af_BNO055::VECTOR_LINEARACCEL);

    event->acceleration.x = vec.x();
    event->acceleration.y = vec.y();
    event->acceleration.z = vec.z();
  } else if (vec_type == Af_BNO055::VECTOR_ACCELEROMETER) {
    event->type = SENSOR_TYPE_ACCELEROMETER;
    vec = getVector(Af_BNO055::VECTOR_ACCELEROMETER);

    event->acceleration.x = vec.x();
    event->acceleration.y = vec.y();
    event->acceleration.z = vec.z();
  } else if (vec_type == Af_BNO055::VECTOR_GRAVITY) {
    event->type = SENSOR_TYPE_GRAVITY;
    vec = getVector(Af_BNO055::VECTOR_GRAVITY);

    event->acceleration.x = vec.x();
    event->acceleration.y = vec.y();
    event->acceleration.z = vec.z();
  } else if (vec_type == Af_BNO055::VECTOR_EULER) {
    event->type = SENSOR_TYPE_ORIENTATION;
    vec = getVector(Af_BNO055::VECTOR_EULER);

    event->orientation.x = vec.x();
    event->orientation.y = vec.y();
    event->orientation.z = vec.z();
  } else if (vec_type == Af_BNO055::VECTOR_GYROSCOPE) {
    event->type = SENSOR_TYPE_GYROSCOPE;
    vec = getVector(Af_BNO055::VECTOR_GYROSCOPE);

    event->gyro.x = vec.x() * SENSORS_DPS_TO_RADS;
    event->gyro.y = vec.y() * SENSORS_DPS_TO_RADS;
    event->gyro.z = vec.z() * SENSORS_DPS_TO_RADS;
  } else if (vec_type == Af_BNO055::VECTOR_MAGNETOMETER) {
    event->type = SENSOR_TYPE_MAGNETIC_FIELD;
    vec = getVector(Af_BNO055::VECTOR_MAGNETOMETER);
 ...
 */
bool Af_BNO055::getSensorOffsets(uint8_t *calibData) {
  if (isFullyCalibrated()) {
    af_bno055_opmode_t lastMode = _mode;
    setMode(OPERATION_MODE_CONFIG);
 ...
 *  @return true if read is successful
 */
bool Af_BNO055::getSensorOffsets(
    af_bno055_offsets_t &offsets_type) {
  if (isFullyCalibrated()) {
    af_bno055_opmode_t lastMode = _mode;
    setMode(OPERATION_MODE_CONFIG);
    delay(25);
 ...
 *          calibration data
 */
void Af_BNO055::setSensorOffsets(const uint8_t *calibData) {
  af_bno055_opmode_t lastMode = _mode;
  setMode(OPERATION_MODE_CONFIG);
  delay(25);
 ...
 */
void Af_BNO055::setSensorOffsets(
    const af_bno055_offsets_t &offsets_type) {
  af_bno055_opmode_t lastMode = _mode;
  setMode(OPERATION_MODE_CONFIG);
  delay(25);
 ...
 */
bool Af_BNO055::isFullyCalibrated() {
 ...
 */
void Af_BNO055::enterSuspendMode() {
  af_bno055_opmode_t modeback = _mode;
 ...
 */
void Af_BNO055::enterNormalMode() {
  af_bno055_opmode_t modeback = _mode;
 ...
 */
bool Af_BNO055::write8(af_bno055_reg_t reg, byte value) {
 ...
 */
byte Af_BNO055::read8(af_bno055_reg_t reg) {
 ...
 */
bool Af_BNO055::readLen(af_bno055_reg_t reg, byte *buffer,
                              uint8_t len) {
